...
- **완료**: Phase 0(엔진 보강) + 0.5(wgpu 22 + NotoSansKR 동봉) + **Phase 1 전체 (Vertical Slice MVP)** — 1-A(진입점·Player·Camera) + 1-B(Zombie·스폰) + 1-C(Whip·DamageSystem·HitFlash) + 1-D(XpGem·자석) + 1-E(레벨업·카드 선택) + **1-F(HUD·사망·GameOver·R 재시작)** + **Phase 2-A(WeaponInventory + Whip 마이그레이션)** + **Phase 2-B(ProjectileSystem + Magic Wand)**
- **상태**: engine 26 tests · game lib 26 tests · doc 2 tests · binary 3개(`game`/`text_demo`/`survivor`) 모두 빌드 통과
- **달성**: Projectile 컴포넌트 + ProjectileSystem(이동·충돌·데미지·despawn). MagicWandSystem(가장 가까운 적 1마리 직선 투사체). with_starter_loadout (Whip+MagicWand 동시 장착).
