...
mod tests {
    ...
    fn mouse_press_sets_state() {
        ...
        assert!(input.is_mouse_pressed(MouseButton::Left),   "press 후 is_mouse_pressed 가 true 여야 한다");
        assert!(input.mouse_just_pressed(MouseButton::Left), "press 후 mouse_just_pressed 가 true 여야 한다");

        input.flush();

        assert!(input.is_mouse_pressed(MouseButton::Left),    "flush 후에도 누르고 있으면 is_mouse_pressed 는 true");
        assert!(!input.mouse_just_pressed(MouseButton::Left), "flush 후 mouse_just_pressed 는 false 여야 한다");
    }
    ...
    fn mouse_release_clears_pressed() {
        ...
        assert!(!input.is_mouse_pressed(MouseButton::Left),    "release 후 is_mouse_pressed 는 false");
        assert!(input.mouse_just_released(MouseButton::Left),  "release 후 mouse_just_released 는 true");

        input.flush();

        assert!(!input.mouse_just_released(MouseButton::Left), "flush 후 mouse_just_released 는 false");
    }
    ...
    fn mouse_press_twice_no_repeat() {
        ...
        assert!(input.is_mouse_pressed(MouseButton::Left),    "두 번 눌러도 pressed=true");
        assert!(input.mouse_just_pressed(MouseButton::Left),  "첫 press 의 just_pressed 는 유지");

        // flush 후 just_pressed 리셋, 다시 press → 이미 pressed=true 이므로 just_pressed 켜지지 않음
        input.flush();
        input.press_mouse(MouseButton::Left);
        assert!(!input.mouse_just_pressed(MouseButton::Left), "이미 눌린 상태에서 press 해도 just_pressed=false");
    }
    ...
    fn scroll_accumulates_and_resets() {
        ...
        assert!((input.scroll() - 3.5).abs() < f32::EPSILON, "scroll 은 누적되어야 한다");
