...
**현재 단계**: Phase 1-C (Whip 무기 + DamageSystem)
...
| `weapon.rs` | `Whip` 컴포넌트, `WhipSystem` — 1초 cooldown, 좌/우 AABB, LAYER_ENEMY 쿼리, despawn |
...
입력 → 플레이어 이동 → 적 추격 → 스폰/정리 → 카메라 follow → 무기 순서를 유지한다.
