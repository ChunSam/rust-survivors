    ...
    fn run(&mut self, world: &mut World, dt: f32) {
        ...
        let pools: Vec<(Entity, Vec2, f32, f32, f32, f32, bool)> = world
            ...
            .map(|(e, pool, t)| {
                ...
                (e, t.position, pool.damage, pool.radius, new_life, new_tick, fired)
            })
            .collect();
        ...
        for (e, pos, damage, radius, life, tick_elapsed, fired) in pools {
            ...
            if fired {
                let zombies = self.grid.query_radius(pos, radius, CollisionLayer(LAYER_ENEMY));
                for z in zombies {
...
pub fn spawn_holy_water_pool(
    ...
    world.add_component(e, Sprite::colored(0.4, 0.6, 1.0)); // 연한 파랑
    world.add_component(e, HolyWaterPool {
        damage,
        radius,
        lifetime,
        tick_cooldown,
