...
impl System for KingBibleSystem {
    fn run(&mut self, world: &mut World, dt: f32) {
        ...
        let fire_info = {
            ...
            let Some(slot) = inv.king_bible_slot_mut() else { return };
            if !slot.tick_with_cooldown_multiplier(dt, stats.cooldown) { return; }
            if let WeaponKind::KingBible { damage, book_count, radius, angular_speed, lifetime, tick_cooldown, hit_radius } = slot.kind {
                Some((damage, book_count, radius, angular_speed, lifetime, tick_cooldown, hit_radius))
            } else { None }
        };
        let Some((base_damage, base_book_count, base_radius, angular_speed, base_lifetime, tick_cooldown, base_hit_radius)) = fire_info else { return };
        ...
        for i in 0..book_count {
            ...
            spawn_book(world, player_pos, angle, angular_speed, radius, damage, hit_radius, lifetime, tick_cooldown);
        }
    }
}
...
impl Default for OrbitingBookSystem {
    fn default() -> Self { Self { grid: SpatialGrid::new(128.0) } }
}
...
pub fn spawn_book(
    ...
    world.add_component(e, Sprite::colored(0.6, 0.8, 1.0)); // 연한 청색
    world.add_component(e, OrbitingBook {
        angle,
        angular_speed,
        radius,
        damage,
