...
pub fn load_starter_weapons() -> Vec<WeaponSlot> {
    let parsed: WeaponsDataFile = ron::from_str(WEAPONS_RON)
        .expect("assets/data/weapons.ron 파싱 실패");
...
fn weapon_def_to_slot(def: &WeaponDef) -> WeaponSlot {
    let kind = match def.kind.as_str() {
        ...
        "MagicWand" => WeaponKind::MagicWand {
            ...
            projectile_speed: def.projectile_speed.expect("MagicWand: projectile_speed 필드 필요"),
        ...
        "Knife" => WeaponKind::Knife {
            ...
            projectile_speed: def.projectile_speed.expect("Knife: projectile_speed 필드 필요"),
        ...
        "Cross" => WeaponKind::Cross {
            ...
            projectile_speed: def.projectile_speed.expect("Cross: projectile_speed 필드 필요"),
        ...
        "FireWand" => WeaponKind::FireWand {
            ...
            projectile_speed: def.projectile_speed.expect("FireWand: projectile_speed 필드 필요"),
        ...
        "HolyWater" => WeaponKind::HolyWater {
            ...
            pool_lifetime: def.pool_lifetime.expect("HolyWater: pool_lifetime 필드 필요"),
            tick_cooldown: def.tick_cooldown.expect("HolyWater: tick_cooldown 필드 필요"),
        ...
        "KingBible" => WeaponKind::KingBible {
            ...
            angular_speed: def.angular_speed.expect("KingBible: angular_speed 필드 필요"),
            lifetime:      def.lifetime.expect("KingBible: lifetime 필드 필요"),
            tick_cooldown: def.tick_cooldown.expect("KingBible: tick_cooldown 필드 필요"),
        ...
        "LightningRing" => WeaponKind::LightningRing {
            ...
            strike_count: def.strike_count.expect("LightningRing: strike_count 필드 필요"),
...
mod tests {
    ...
    fn weapons_ron_loads_ten_weapons() {
        ...
        // 순서대로 각 variant 검증
        assert!(matches!(slots[0].kind, WeaponKind::Whip { .. }),         "슬롯 0: Whip");
        assert!(matches!(slots[1].kind, WeaponKind::MagicWand { .. }),     "슬롯 1: MagicWand");
        assert!(matches!(slots[2].kind, WeaponKind::Knife { .. }),         "슬롯 2: Knife");
        assert!(matches!(slots[3].kind, WeaponKind::Axe { .. }),           "슬롯 3: Axe");
        assert!(matches!(slots[4].kind, WeaponKind::Cross { .. }),         "슬롯 4: Cross");
        assert!(matches!(slots[5].kind, WeaponKind::FireWand { .. }),      "슬롯 5: FireWand");
        assert!(matches!(slots[6].kind, WeaponKind::Garlic { .. }),        "슬롯 6: Garlic");
        assert!(matches!(slots[7].kind, WeaponKind::HolyWater { .. }),     "슬롯 7: HolyWater");
        assert!(matches!(slots[8].kind, WeaponKind::KingBible { .. }),     "슬롯 8: KingBible");
        assert!(matches!(slots[9].kind, WeaponKind::LightningRing { .. }), "슬롯 9: LightningRing");

        // 대표 스탯 검증
        assert!(matches!(slots[0].kind, WeaponKind::Whip { damage, .. } if (damage - 10.0).abs() < 0.001));
        assert_eq!(slots[0].cooldown, 1.0);
        assert!(matches!(slots[1].kind, WeaponKind::MagicWand { damage, pierce, .. } if (damage - 8.0).abs() < 0.001 && pierce == 0));
        assert!(matches!(slots[9].kind, WeaponKind::LightningRing { strike_count, .. } if strike_count == 1));
    }
}
